...
export default function CaCompanyDetailPage() {
  ...
  useEffect(() => {
    ...
    getCaAuditPack(companyId)
      .then((data) => setAuditPack(data))
      .catch((err) => console.error("Failed to load audit pack:", err))
        ...
        >
          {tasks.length === 0 ? (
            ...
            <>
              {/* Header */}
              <div
                className="grid grid-cols-[1fr_100px_100px_90px_100px] gap-3 px-5 py-3 text-xs font-semibold uppercase tracking-wider"
              ...
              {tasks.map((task, idx) => {
                ...
                return (
                  <div key={task.id}>
                    <div
                      className="grid grid-cols-[1fr_100px_100px_90px_100px] gap-3 px-5 py-3.5 items-center text-sm"
                      style={{
      ...
      {tab === "audit" && (
        <div>
          {auditLoading ? (
            ...
            >
              No audit pack data available for this company.
            </div>
          ) : (
