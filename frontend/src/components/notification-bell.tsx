...
function getNotificationIcon(type: string): string {
  switch (type) {
    case "status_update": return "arrows-right-left";
    case "payment": return "credit-card";
    case "document": return "document-text";
    case "compliance": return "shield-check";
    case "message": return "chat-bubble-left";
    default: return "bell";
  }
}
...
function getNotificationColor(type: string): string {
  switch (type) {
    case "status_update": return "text-blue-400";
...
export default function NotificationBell() {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <svg className="w-5 h-5 text-gray-400 hover:text-white transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" />
        </svg>
          ...
          <div className="max-h-[400px] overflow-y-auto" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(139, 92, 246, 0.3) transparent" }}>
            {loading ? (
              <div className="p-8 text-center">
                <div className="animate-pulse text-gray-500 text-sm">Loading notifications...</div>
              </div>
              ...
              notifications.map((notif) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 flex items-start gap-3 hover:bg-white/5 transition-colors border-b ${
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className={`text-sm font-medium truncate ${!notif.is_read ? "text-white" : "text-gray-300"}`}>
                        {notif.title}
                      </p>
                      {!notif.is_read && (
